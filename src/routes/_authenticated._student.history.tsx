...
export const Route = createFileRoute("/_authenticated/_student/history")({
...
function StudentHistory() {
  ...
  const bySubject = myTx.reduce(
    (acc, tx) => {
      acc[tx.subjectName] = (acc[tx.subjectName] || 0) + tx.pointsAwarded;
      return acc;
    },
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {Object.entries(bySubject).map(([subject, points]) => (
          <div
            key={subject}
            className="flex-shrink-0 rounded-2xl bg-primary/10 px-4 py-3 text-center min-w-[100px]"
          >
              ...
              <CardContent className="flex items-center gap-3 p-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {tx.marksEntered} marks • by {tx.teacherName}
                  </div>
                ...
                <div className="text-right">
                  ...
                  <div className="text-[10px] text-muted-foreground">
                    ({tx.marksEntered}-{tx.passingMarks})×{tx.multiplier}
                  </div>
                </div>
              </CardContent>
            </Card>
