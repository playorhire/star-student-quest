...
function TeacherScan() {
  ...
  const handleMarksChange = (value: string) => {
    ...
    if (subject && !isNaN(numMarks)) { setCalculatedPoints(calculatePoints(numMarks, subject.passingMarks, subject.multiplier)); }
    else { setCalculatedPoints(0); }
  };
  ...
  const handleReset = () => {
    setStep("scanning"); setStudent(null); setSelectedSubjectId(""); setMarks(""); setCalculatedPoints(0); setManualQR("");
  };
      ...
      {step === "scanning" && (
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            <Input placeholder="e.g. STU-s1-101" value={manualQR} onChange={(e) => setManualQR(e.target.value)} className="rounded-xl" />
        ...
        <div className="space-y-4">
          <Card className="border-2 border-success/30 bg-success/5">
            <CardContent className="flex items-center gap-4 p-4">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">Roll #{student.rollNumber} • Class {student.className}</div>
          ...
          <div className="space-y-3">
            ...
            <Select onValueChange={(v) => { setSelectedSubjectId(v); setMarks(""); setCalculatedPoints(0); }} value={selectedSubjectId}>
              ...
              <SelectContent>
                {subjects.filter((s) => s.className === student.className).map((s) => (
                  <SelectItem key={s.id} value={s.id}>{s.name} (Pass: {s.passingMarks}, ×{s.multiplier})</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
                  ...
                  <CardContent className="p-4 text-center">
                    {calculatedPoints > 0 ? (
                      <>
                        ...
                        <div className="text-sm text-muted-foreground">({marks} - {selectedSubject.passingMarks}) × {selectedSubject.multiplier}</div>
                      ...
                      <>
                        ...
                        <div className="text-sm text-muted-foreground">Marks below passing ({selectedSubject.passingMarks})</div>
