...
function StudentDashboard() {
  ...
  const progressToNext = nextBadge
    ? ((student.totalPoints - (earnedBadges.length > 0 ? earnedBadges[earnedBadges.length - 1].requiredPoints : 0)) /
        (nextBadge.requiredPoints - (earnedBadges.length > 0 ? earnedBadges[earnedBadges.length - 1].requiredPoints : 0))) *
      100
    : 100;
        ...
        <CardContent className="p-6 text-center">
          ...
          <div className="flex items-center justify-center gap-4 mt-4">
            <div className="flex items-center gap-1">
              ...
              <span className="text-sm font-bold text-secondary">#{rank}</span>
              <span className="text-xs text-muted-foreground">rank</span>
            </div>
            <div className="flex items-center gap-1">
              <Flame className="h-4 w-4 text-accent" />
              <span className="text-sm font-bold text-accent">{earnedBadges.length}</span>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-sm font-bold text-card-foreground">{nextBadge.name}</div>
                  <div className="text-[10px] text-muted-foreground">{nextBadge.description}</div>
                </div>
              </div>
              <span className="text-xs font-semibold text-muted-foreground">
                {student.totalPoints}/{nextBadge.requiredPoints}
              </span>
            </div>
